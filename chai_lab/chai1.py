...
def make_all_atom_feature_context(
    ...
    *,
    ...
    if pdb_conformer_path is not None:
        chains = load_chains_from_raw(fasta_inputs, entity_name_as_subchain=entity_name_as_subchain, pdb_conformer_path=pdb_conformer_path)
    else:
        chains = load_chains_from_raw(fasta_inputs, entity_name_as_subchain=entity_name_as_subchain)
...
def run_folding_on_context(
    ...
    *,
    ...
    # Initial atom positions TODO: save traj
    ...
    """
    assuming num_diffn_samples=3, confidence_head returns a tuple of (pae, pde, plddt)
    confidence_outputs = [
        (pae1, pde1, plddt1),  # Sample 1
        (pae2, pde2, plddt2),  # Sample 2
        (pae3, pde3, plddt3)   # Sample 3
    ]

    zip(*confidence_outputs) is equivalent to:
    [
        (pae1, pae2, pae3), 
        (pde1, pde2, pde3), 
        (plddt1, plddt2, plddt3)
    ]

    Concatenate each type:
    pae_logits = torch.cat([pae1, pae2, pae3], dim=0)   # shape: (3, ...)
    pde_logits = torch.cat([pde1, pde2, pde3], dim=0)   # shape: (3, ...)
    plddt_logits = torch.cat([plddt1, plddt2, plddt3], dim=0)  # shape: (3, ...)
    """
