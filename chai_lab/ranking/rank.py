...
def rank(
    ...
    # pde (optional)
    pde_logits: Optional[Float[Tensor, "... n n pde_bins"]] = None,
    # clash
...
def get_scores(ranking_data: SampleRanking, *, pae_scores: np.ndarray | None = None, pde_scores: np.ndarray | None = None) -> dict[str, np.ndarray]:
    """Get all scores from ranking data and additional metrics

    Args:
        ranking_data: Ranking data containing PTM scores and clash scores
        pae_scores: Optional predicted aligned error scores
        pde_scores: Optional predicted distance error scores
    ...
    scores = {
        ...
        "actif_ptm": ranking_data.ptm_scores.actif_ptm,
        "mean_interface_ptm": ranking_data.ptm_scores.mean_interface_ptm,
        "protein_mean_interface_ptm": ranking_data.ptm_scores.protein_mean_interface_ptm,
    }
    
    # Add optional scores if provided
    if pae_scores is not None:
        scores["pae_scores"] = pae_scores
    if pde_scores is not None:
        scores["pde_scores"] = pde_scores

    return {k: v.cpu().numpy() if torch.is_tensor(v) else v for k, v in scores.items()}
